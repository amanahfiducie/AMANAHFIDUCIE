...
export default function ChariaHomePage() {
  const { user } = useAuth();
  const displayName = useMemo(() => {
    const first = user?.first_name?.trim();
    const last = user?.last_name?.trim();
    if (first || last) return [first, last].filter(Boolean).join(" ");
    return user?.username ?? "Comité";
  }, [user]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [dossiers, queue, observations, cases] = await Promise.all([
        ...
        if (cancelled) return;
        // my-queue = étapes assignées à mon rôle (comité) ; CHARIA = demandes typées.
        const demandes = queue.filter((v) => v.validation_type === "CHARIA");
        const circuits = dossiers.filter(
          (v) =>
            v.can_decide
            || v.current_step?.assigned_role === "COMITE_CHARAIQUE",
        );
        setState({
          circuits,
          demandes,
          observationsPending: observations.length,
          successions: cases.filter((c) => c.case_type === "SUCCESSION").length,
          loading: false,
          error: null,
        });
      } catch (err) {
  ...
  const pendingTotal =
    state.circuits.length + state.demandes.length + state.observationsPending;

  const cards = [
    {
      href: "/charia/circuits",
      label: "Circuits dossiers",
      hint: "Votre étape comité",
      value: state.circuits.length,
    },
    {
      href: "/charia/demandes",
      label: "Demandes CHARIA",
      hint: "Avis à émettre",
      value: state.demandes.length,
    },
    {
      href: "/charia/observations",
      label: "Observations",
      hint: "À examiner",
      value: state.observationsPending,
    },
    {
      href: "/charia/partages",
      label: "Partages farāʾiḍ",
      hint: "Dossiers succession",
      value: state.successions,
    },
  ];
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--sf-green)]/55">
          Espace comité charaïque — uniquement les dossiers et décisions qui vous
          concernent.
          {!state.loading ? (
            <>
              {" "}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <section className="rounded-xl border border-[var(--sf-cream-dark)] bg-white p-5 shadow-sm">
            ...
            {previewDemandes.length === 0 ? (
              <p className="text-sm text-[var(--sf-green)]/45">
                Aucune demande charaïque en attente.
              </p>
            ) : (
              <ul className="divide-y divide-[var(--sf-cream-dark)]">
